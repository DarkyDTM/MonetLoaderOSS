...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (init_state && g.ActiveId != id)
    {
        ...
        state->IMEExEndInputWithEnter = state->IMEExEndInputWithUnfocus = state->IMEExNewInput = false; // IME Ex: Reset state
#endif
...
#ifdef NEW_INPUT_SYSTEM
    // IME Ex: Filter characters
    // NOTE: Buffers are allocated in handler
    int imeex_old_len = -1;
    if (g.ActiveId == id && state->IMEExNewInput)
    {
        imeex_old_len = state->CurLenA;
        int offset_selection_start = 0;
        int offset_selection_end = 0;

        int skipped_characters = 0;
        int wide_offset = 0;
        int wide_write_ptr = 0;

        for (const char* s = state->TextA.Data; *s; )
        {
            unsigned int c;
            ...
            if (c >= 0x10000 || !InputTextFilterCharacter(&c, flags, callback, callback_user_data))
            {
                ...
                if (wide_offset <= state->Stb.select_start)
                {
                    offset_selection_start = skipped_characters;
                }
                if (wide_offset <= state->Stb.select_end)
                {
                    offset_selection_end = skipped_characters;
                }
                continue;
            }

            state->TextW.Data[wide_write_ptr++] = (ImWchar)c;
            ++wide_offset;
        }

        state->TextW.Data[wide_write_ptr] = 0;

        state->CurLenW = wide_write_ptr;
        state->CurLenA = ImTextStrToUtf8(state->TextA.Data, state->TextA.Size,
            state->TextW.Data, state->TextW.Data + wide_write_ptr);

        state->Stb.select_start -= offset_selection_start;
        state->Stb.select_end -= offset_selection_end;
        state->Stb.cursor -= offset_selection_end;
        state->CursorClamp();

        state->IMEExNewInput = false;
    }
#endif
...
#ifdef NEW_INPUT_SYSTEM
        backup_current_text_length = imeex_old_len < 0 ? state->CurLenA : imeex_old_len;
        state->IMEExFlags = flags;
#else
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        // IME Ex: Enter handler
        else if (IsKeyPressedMap(ImGuiKey_Enter) || IsKeyPressedMap(ImGuiKey_KeyPadEnter)
#ifdef NEW_INPUT_SYSTEM
            || state->IMEExEndInputWithEnter
#endif
        )
        {
#ifdef NEW_INPUT_SYSTEM
            state->IMEExEndInputWithEnter = false;
#endif
...
#ifdef NEW_INPUT_SYSTEM
        else if (state->IMEExEndInputWithUnfocus) // IME Ex: Unfocus handler
        {
            clear_active_id = true;
        }
#endif
