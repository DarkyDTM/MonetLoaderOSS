...
script_description('Script manager that opens on left swipe on radar and provides ability to receive notifications and get script info.')
...
-- simple filterer class (for list clipping)

local Filterer = {}
local Filterer.metatable = {}

function Filterer.deactivate(self)
  self.buffer = {}
  self.enabled = false
  self.filter = ''
  self.front = 1
...
function Filterer.activate(self, buffer, filter)
...
function Filterer.run(self, buffer)
  self.buffer = {}
  self.front = 1
  self.back = #buffer

  for i = 1, #buffer do
    if buffer[i]:lower():find(self.filter, 1, true) then
      self.buffer[i] = buffer[i]
    end
  end
end

function Filterer.push(self, el, needPop)
  if needPop then
    self.buffer[self.front] = nil
    self.front = self.front + 1
  end

  if el:lower():find(self.filter, 1, true) then
    self.buffer[self.back] = el
    self.back = self.back + 1
  end
end

function Filterer.metatable.__index(self, i)
  return self.buffer[i + self.front - 1]
end

function Filterer.metatable.__len(self)
  return self.back - self.front
end

function Filterer.new()
  local instance = {
    deactivate = Filterer.deactivate,
    activate = Filterer.activate,
    run = Filterer.run,
    push = Filterer.push
  }
  instance:deactivate()
  return setmetatable(instance, Filterer.metatable)
end
...
local filteredMessages = Filterer.new()
    ...
    if imgui.BeginTabBar('Tabs') then
      ...
      if imgui.BeginTabItem('Scripts') then -- common scripts control
        ...
        if scr ~= nil then
          ...
          imgui.Text('Select any script on the left!')
        end
      ...
      if imgui.BeginTabItem('Log') then -- log of recent events
        if imgui.InputTextWithHint('##LogSearch', 'Find...', logSearchBuffer, ffi.sizeof(logSearchBuffer)) then
          local text = ffi.string(logSearchBuffer)
          if #text == 0 then
            filteredMessages:deactivate()
          else
            filteredMessages:activate(messages, ffi.string(logSearchBuffer))
          end
        end
        imgui.SameLine()
        if imgui.Button('Clear history') then
          imgui.StrCopy(logSearchBuffer, '')
          messages:clear()
          filteredMessages:deactivate()
        end
        ...
        local t = filteredMessages.enabled and filteredMessages or messages
        local clipper = imgui.ImGuiListClipper(#t)
...
function onScriptCrashed(scr, msg)
  ...
  local msg = '(crash) ' .. scr.name .. ': ' .. msg
  filteredMessages:push(msg, messages:filled())
  messages:push(msg)
end
