...
#define HOOK_ITEM_COUNT 4096
struct inlineHookInfo {
	struct inlineHookItem item[HOOK_ITEM_COUNT];
	int size;
};

static struct inlineHookInfo info = {0};

#ifndef ARM_INLINE_HOOK_NO_FREEZE
...
#ifdef ARM_INLINE_HOOK_DO_EXECUTABLE_CHECK
...
{
	memmove(&info.item[pos], &info.item[pos + 1], sizeof(struct inlineHookItem) * (HOOK_ITEM_COUNT - 1 - pos));
	--info.size;
}
...
#ifndef ARM_INLINE_HOOK_NO_FREEZE
	pid_t pid = freeze(NULL, ACTION_DISABLE);
#endif
