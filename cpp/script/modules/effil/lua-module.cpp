...
int luaopen_effil(lua_State* L) {
    ...
    if (registry["effil.EFFIL_LOADED"] == true) {
        sol::stack::push(lua, EffilApiMarker());
        return 1;
    }
    registry["effil.EFFIL_LOADED"] = true;
    
    sol::table dummy_table = lua.create_table();
    Thread::exportAPI(dummy_table);
    SharedTable::exportAPI(dummy_table);
    Channel::exportAPI(dummy_table);
    ThreadRunner::exportAPI(dummy_table);
    ...
    dummy_table.new_usertype<EffilApiMarker>(
        "effil.EffilApiMarker",
