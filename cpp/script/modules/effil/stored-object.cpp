...
namespace effil {
template<class T>
static TypeId getTypeIdInternal() //this function is instantiated for every different type
{
...
{
    return getTypeIdInternal<typename std::remove_cv<typename std::remove_reference<T>::type>::type>();
}
...
public:
    ApiReferenceHolder()
        : BaseHolder(getTypeId<decltype(*this)>())
    {
    }
...
public:
    ...
    {
      // dynamic_cast removed since compare checks typeid before checking rawCompare
      const auto cfh = static_cast<const CFunctionHolder*>(other);
      return cfh->cfunction_ == cfunction_;
    }
...
{
  if (sobj->type_id != getTypeId<PrimitiveHolder<DataType>>()) {
    return sol::optional<DataType>();
  }
  return static_cast<PrimitiveHolder<DataType>*>(sobj.get())->getData();
}
...
{
  // dynamic_cast replaced with type_id check
  if (obj->type_id == getTypeId<GCObjectHolder<SharedTable>>()) {
    return GC::instance().get<SharedTable>(obj.get()->gcHandle());
  }
  return sol::nullopt;
}
...
{
  // dynamic_cast replaced with type_id check
  if (obj->type_id == getTypeId<GCObjectHolder<Function>>()) {
    return GC::instance().get<Function>(obj.get()->gcHandle());
  }
  return sol::nullopt;
}
